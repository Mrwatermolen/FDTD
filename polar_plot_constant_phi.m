function polar_plot_constant_phi(theta, pattern_1, pattern_2, ...
    ...
    hold on;
    th = 0:(pi / 50):2 * pi; circlex = cos(th); circley = sin(th);

    for mi = 1:number_of_rings
        r = (1 / number_of_rings) * mi;
        plot(r * circlex, r * circley, ':', 'color', 'k', 'linewidth', 1);
    ...
    for mi = 3:8
        ...
        text(1.1 * cos (th), 1.1 * sin(th), [num2str(30 * (mi - 3))], 'horizontalalignment', "center", "Color", 'k', "FontWeight", "demi", "FontSize", 10);
    ...
    pattern_1(pattern_1 < min_val) = min_val;
    pattern_1 = (pattern_1 - min_val) / plot_range;
    pattern_2(pattern_2 < min_val) = min_val;
    pattern_2 = (pattern_2 - min_val) / plot_range;

    %transformdatatoCartesiancoordinates
    ...
    if constant_phi == 0
        ...
        text(1.2, 0, '¹', 'fontname', 'symbol', 'color', 'b', 'fontweight', 'demi');
        text(0.06, 1.23, 'z', 'fontname', 'arial', 'color', 'b', 'fontweight', 'demi');
        text(0, 1.23, '¼', 'fontname', 'symbol', 'color', 'b', 'fontweight', 'demi');
        text(1.2 * cos(pi / 4), 1.18 * sin(pi / 4) - 0.12, 'xzplane', ...
            'color', 'b', 'fontweight', 'demi');
    end
    ...
    if constant_phi == 90
        ...
        text(0.06, 1.23, 'z', 'fontname', 'arial', 'color', 'b', 'fontweight', 'demi');
        text(0, 1.23, '¼', 'fontname', 'symbol', 'color', 'b', 'fontweight','demi');
        text(1.2 * cos(pi / 4), 1.18 * sin(pi / 4) - 0.12, 'yzplane', ...
            'color', 'b', 'fontweight', 'demi');
    end
